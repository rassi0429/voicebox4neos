...
app.get("/speakers", async (req, res) => {
    const { data } = await axios.get(`http://${process.env.VOICE_HOST || "localhost"}:50021/speakers`)
...
const createVoice = async (text, hash, speakerId) => {
    console.log("cq")
    const query = await client.query.createQuery(speakerId, text)
    console.log("cv")
    const voice = await client.voice.createVoice(speakerId, query)
    console.log("cv ok")
